...
export function WorkspaceLayout({
    ...
    const openUserProfile = (userName: string) => {
        ...
        if (member && userProfile && member.name === userProfile.displayName) {
            openMyProfile();
            return;
        }
    ...
    if (isInMeeting) {
        return (
            <div className="flex h-screen w-full relative bg-slate-950">
                ...
                <div
                    className={`bg-white border-l transition-all duration-300 ease-in-out absolute right-0 top-0 bottom-0 z-20 ${isChatOpenInMeeting ? 'w-96 translate-x-0 shadow-2xl' : 'w-0 translate-x-full overflow-hidden'
                        }`}
                >
                            ...
                            <TooltipTrigger asChild>
                                ...
                                >
                                    {userProfile?.avatarUrl ? (
                                        <img src={userProfile.avatarUrl} className="w-7 h-7 rounded-full" alt="Home" />
                                    ) : (
                                        <span className="text-sm font-bold">{userProfile?.displayName?.charAt(0) || 'U'}</span>
                                    )}
                                </div>
                            </TooltipTrigger>
                    ...
                    <aside className="w-[240px] bg-[#2B2D31] text-[#949BA4] flex flex-col shrink-0 relative z-20 rounded-tl-xl my-0 ml-0 overflow-hidden">
                        ...
                        <ScrollArea className="flex-1 px-2 py-4">
                            ...
                            <div className="mb-6">
                                ...
                                {channels.filter(c => ['text', 'announcement'].includes(c.type)).map((channel, index) => (
                                    <DraggableChannelItem
                                        ...
                                        moveChannel={(dragIdx, hoverIdx) => moveChannel(dragIdx, hoverIdx, ['text', 'announcement'])}
                        ...
                        <div className="h-[52px] bg-[#232428] flex items-center px-2 gap-2">
                            ...
                            <div className="flex-1 overflow-hidden cursor-pointer" onClick={openMyProfile}>
                                <div className="text-sm font-bold text-white truncate leading-tight hover:underline">{userProfile?.displayName || 'User'}</div>
                                        ...
                                        {roleMembers.map(member => (
                                            ...
                                            >
                                                <div className="relative">
                                                    ...
                                                    <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-[#F2F3F5] ${member.status === 'online' ? 'bg-green-500' :
...
function ChannelItem({ channel, isActive, onClick, onSettings }: {
    ...
    return (
        <div
            onClick={onClick}
            className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-md mb-0.5 text-[15px] transition-all group relative cursor-pointer ${isActive
...
// Draggable Channel Item - with drag and drop support
const CHANNEL_ITEM_TYPE = 'CHANNEL_ITEM';
...
function DraggableChannelItem({ channel, index, isActive, onClick, onSettings, moveChannel, channelTypes }: DraggableChannelItemProps) {
    const ref = useRef<HTMLDivElement>(null);

    const [{ isDragging }, drag] = useDrag({
        type: `${CHANNEL_ITEM_TYPE}_${channelTypes.join('_')}`,
        item: { index },
        collect: (monitor) => ({
    ...
    const [{ isOver }, drop] = useDrop({
        accept: `${CHANNEL_ITEM_TYPE}_${channelTypes.join('_')}`,
    ...
    return (
        <div
            ref={ref}
            onClick={onClick}
            className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-md mb-0.5 text-[15px] transition-all group relative cursor-grab active:cursor-grabbing ${isDragging ? 'opacity-50 bg-[#404249]' : ''
                } ${isOver ? 'bg-[#35373C]' : ''} ${isActive
                    ? 'bg-[#404249] text-white font-medium'
                    : 'hover:bg-[#35373C] hover:text-[#DBDEE1]'
                }`}
        ...
        <Tooltip>
            <TooltipTrigger asChild>
                <div className="relative flex items-center group w-full justify-center">
                    ...
                    {!isSmall && (
                        <div className={`absolute left-0 w-[4px] bg-white rounded-r-full transition-all duration-200 ${isActive ? 'h-[40px]' : 'h-[8px] opacity-0 group-hover:opacity-100 group-hover:h-[20px]'
                            }`} />
                    )}
                    {isSmall && isActive && (
                    ...
                    <button
                        onClick={onClick}
                        className={`${sizeClasses} flex items-center justify-center rounded-[24px] transition-all duration-200 overflow-hidden ${isActive
