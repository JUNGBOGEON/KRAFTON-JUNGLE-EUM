...
export function TranslationChat({ onOpenProfile, onOpenMyProfile }: TranslationChatProps) {
  ...
  // Handle profile click - distinguishes between my profile and others
  const handleProfileClick = (msg: Message) => {
    if (msg.sender.id === 'me') {
      // Open my profile (without "send message" option)
      onOpenMyProfile?.();
    } else {
      // Open other user's profile
      onOpenProfile?.(msg.sender.name);
    }
  };

  // Get display info for a message - use real-time userProfile for 'me' messages
            ...
            return (
              <div key={msg.id} className={`flex gap-3 ${msg.sender.id === 'me' ? 'flex-row-reverse' : ''}`}>
                ...
                >
                  ...
                  <AvatarFallback className="text-xs text-white" style={{ backgroundColor: sender.color }}>
                    {sender.name[0]}
                  </AvatarFallback>
                </Avatar>
                <div className={`flex flex-col max-w-[80%] ${msg.sender.id === 'me' ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`p-3 rounded-lg text-sm shadow-sm relative group ${msg.sender.id === 'me'
